...
questions_category<-c('All Questions',unique(questions_table$Question_category))
...
get_summary_objects<-function(client_name, report_year){
  ...
  summarize_favorability_plot<-generate_favorability_doughnut_plot(formatted_results,color_palette2)
  summarize_participation_plot<-generate_participation_pie_chart(formatted_results)
  objects_list<- list(lowest_scores = lowest_scores,
...
demographics_participant_stats_plot<-function(profile_revised,attribute){
...
analyze_demographics<-function(demographics_table, attribute,query_type,profile_dataset){
  ...
  for(n in 2:ncol(temp_df)){
    ...
    g<-ggplot(df4,aes(x=question,y=parameter,label=parameter)) +
      geom_segment(aes(yend=0,xend=question),
                   linewidth=2,color=color_palette[4]) +   
      geom_point(
      ...
      theme(legend.position = 'none',
            ...
            axis.title.x=element_text(size=12,family="Arial",
                                      color=color_palette[1]),  # Y axis title
...
generate_Question_category_plots<-function(df,question_type){
  ...
  for (x in 1:nrow(data_filtered)){
    ...
    g<-ggplot(data, aes(x=factor(question),
      ...
      labs(title=strwrap("Score Distribution"), x="Response", y="Percentage", 
           subtitle = question_number, caption = wrapper(merged_question))
    g
    list_of_graphs[[x]]<-g
  }
...
get_detailed_objects<-function(client_name,report_year,question_type){
  ...
  detailed_questions_table<-get_detailed_analysis_2(formatted_results,
                                                    question_type,
                                                    summary_demographics_table)
  
  questions_filtered_table<-get_questions(question_type)
  distribution_scores_plots<-get_score_distribution_plot(
    calculate_all_questions(formatted_results)
    )
  objects_list<-list(
